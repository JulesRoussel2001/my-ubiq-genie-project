...
class ServiceController extends EventEmitter {
    ...
    sendToChildProcessAsync(identifier: string, data: string | Buffer): Promise<Buffer> {
        return new Promise((resolve) => {
            const onResponse = (response: Buffer, responseId: string) => {
                if (responseId === identifier) {
                    this.off('data', onResponse);
                    resolve(response);
                }
            };
    
            this.on('data', onResponse);
            this.sendToChildProcess(identifier, data);
        });
    }
